...
class Adjusment_dialog() :
  def __init__(self) :
    self.user_enhancement_choice=False
    
    self.enhance_value=False
    
  def get_user_enhancement_type_choice(self,widget,event=False) :
    if widget.get_name() == "Colors" :
  ...
  def get_user_enhancement_color_value_change(self,widget,event=False) :
    self.enhance_value=self.color_adjustment_value_adjustment.get_value()
   
  def get_user_enhancement_brightness_value_change(self,widget,event=False) :
    self.enhance_value=self.brightness_adjustment_value_adjustment.get_value()  
  
  def get_user_enhancement_contrast_value_change(self,widget,event=False) :
    self.enhance_value=self.contrast_adjustment_value_adjustment.get_value()
  
  def get_user_enhancement_sharpness_value_change(self,widget,event=False) :
    self.enhance_value=self.sharpness_adjustment_value_adjustment.get_value()  
  
  
  def create_dialog(self) :
    global adjustment_type,value
    
    adjustment_type,value=1,0.5
    ...
    self.color_adjustment_value_adjustment=gtk.Adjustment(value=0.5, lower=0.0, upper=1.0, step_incr=0.001, page_incr=0, page_size=0)
    ...
    self.color_adjustment_value_scale.add_mark(0.5, gtk.POS_RIGHT, None)
    ...
    self.brightness_adjustment_value_adjustment=gtk.Adjustment(value=0.5, lower=0.0, upper=1.0, step_incr=0.001, page_incr=0, page_size=0)
    ...
    self.brightness_adjustment_value_scale.add_mark(0.5, gtk.POS_RIGHT, None)
    ...
    self.contrast_adjustment_value_adjustment=gtk.Adjustment(value=0.5, lower=0.0, upper=1.0, step_incr=0.001, page_incr=0, page_size=0)
    ...
    self.contrast_adjustment_value_scale.add_mark(0.5, gtk.POS_RIGHT, None)
    ...
    self.enhancement_dialog_action_apply_button.connect("button-press-event",self.apply_adjustment)
  ...
  def cancel(self,widget,event) :
    global adjustment_type,value
    adjustment_type,value=False,False
    closed(self.enhancement_dialog,False)
  
  def apply_adjustment(self,widget,event) :
    global adjustment_type,value
    
    adjustment_type=self.user_enhancement_choice
    value=self.enhance_value
